//src\components\GestionAlmacenesPanel.jsx
...
export default function GestionAlmacenesPanel({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch("/api/drive?seccion=gestion");
        ...
        console.error("Error loading info de gestion de almacenes:", err);
      }finally{
      ...
      <MultiLevelDrawer isOpen={isOpen} onClose={onClose}>
        <div className="bg-white rounded-2xl shadow p-4 flex h-[75vh] w-full overflow-x-auto min-h-0">
          ...
          <div className="w-64 border-r p-4 flex flex-col gap-4">
            <h3 className="font-semibold">Categorías de gestión</h3>
              ...
              {leftMenuData.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-md ${
                    activeSubmenu === n.id ? "bg-yellow-50" : "hover:bg-gray-100"
                  }`}
                >
                  {n.label}
                </button>
